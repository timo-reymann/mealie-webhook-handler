...
BUILD_ARGS=-ldflags "-s -w -X github.com/timo-reymann/mealie-webhook-handler/pkg/buildinfo.GitSha=$(COMMIT_REF) -X github.com/timo-reymann/mealie-webhook-handler/pkg/buildinfo.Version=$(VERSION) -X github.com/timo-reymann/mealie-webhook-handler/pkg/buildinfo.BuildTime=$(NOW)"
BIN_PREFIX="dist/mealie-webhook-handler_"
...
build-playground-wasm:
	@GOOS=js GOARCH=wasm go build  -o playground/code.wasm wasm.go
